...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    attention_score = torch. matmul(query, key.transpose(2,3)) / (self.attention_head_size**0.5)

    attention_score = attention_score + attention_mask
    bs, num_attention_heads, seq_len, seq_len = attention_score.shape

    normal_score = torch.nn.functional.softmax(attention_score, dim=-1)

    normal_score = self.dropout(normal_score)

    values = torch.matmul(normal_score, value)

    values = values.transpose(1,2).contiguous().view(bs, seq_len, -1)
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    dropped_output = dropout(transformed)
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    added_value = self.add_norm(hidden_states, attention_value, self.attention_dense, self.attention_dropout, self.attention_layer_norm)

    ffn_output = self.interm_af(self.interm_dense(added_value))

    final_output = self.add_norm(added_value, ffn_output, self.out_dense, self.out_dropout, self.out_layer_norm)
